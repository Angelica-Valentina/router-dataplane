...
## Screenshot-uri
### Subiect1.png
In subiect1.png se vede un ping de pe h0 (192.168.0.2) catre h1 (192.168.1.2), aflate în subretele diferite. Routerul face forwarding IPv4: valideaza cadrul L2, valideaza checksum-ul IPv4, decrementeaza TTL, alege ruta prin LPM si rescrie adresele MAC pentru next-hop. In Wireshark se vad pachetele ICMP Echo Request/Reply care ajung la destinatie, confirmand dirijarea corecta

### Subiect3.png
In subiect3.png, la primul trafic h0 h1, routerul nu are MAC-ul next-hop-ului, deci emite ARP Request (broadcast) si primeste ARP Reply. Dupa reply, routerul retine maparea IP-MAC in cache si trimite pachetul/pachetele aflate in asteptare. La al doilea ping nu mai apare ARP Request de tip Broadcast, ceea ce demonstreaza caching-ul ARP.

### Subiect4.png
In subiect4.png am trimis un ICMP Echo Request cu TTL=1 de pe h0 catre h1. Routerul detecteaza ca TTL ar expira si raspunde catre sursa cu ICMP Time Exceeded. In Wireshark se vede pachetul ICMP de eroare si faptul ca include headerul IP original + 8 bytes din payload. Apoi am trimis un ICMP Request catre o  destinatie ce nu poate fi atinsa si in Wireshark se poate vedea pachetul de tip  ICMP Destination Unrecheable.